...
export const Navbar: React.FC = () => {
  ...
  return (
    ...
    >
      {/* FIXED CONTAINER WIDTH: Added max-w-7xl and mx-auto to fix the stretched design */}
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <span className={`text-xl font-black tracking-tight transition-colors duration-300 ${
              isScrolled || location.pathname !== '/' ? 'text-white' : 'text-[#0F172A] dark:text-white'
            }`}>
          ...
          {/* Mobile Right-side actions (Menu & Theme) */}
          <div className="md:hidden flex items-center gap-2">
            <button
              onClick={() => setIsDark(!isDark)}
              className={`p-2 rounded-lg transition-colors ${
                isScrolled || location.pathname !== '/' ? 'text-slate-300 hover:text-white' : 'text-slate-600 dark:text-slate-300'
              }`}
            >
      ...
      {isOpen && (
        <div className={`md:hidden px-4 pt-4 pb-6 space-y-3 absolute w-full left-0 border-b shadow-xl ${
          ...
          <div className={`pt-4 border-t ${isScrolled || location.pathname !== '/' ? 'border-slate-800' : 'border-slate-100 dark:border-slate-800'}`}>
            <button 
              onClick={() => {
                setIsOpen(false);
                navigate('/register');
